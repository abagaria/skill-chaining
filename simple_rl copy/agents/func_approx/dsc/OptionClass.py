...
class Option(object):
	...
	def is_init_true(self, ground_state):
		...
		return self.rand_seed.random() < self.initiation_classifier.predict_proba(state.reshape(1,-1)).flatten()[-1]
	...
	def is_term_true(self, ground_state):
		...
		if self.termination_classifier is not None and self.parent is not None:
			state = ground_state.features()[:2] if isinstance(ground_state, State) else ground_state[:2]				
