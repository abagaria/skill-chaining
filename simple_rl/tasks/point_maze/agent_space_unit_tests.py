...
def plot_door_distances(traj, ego_traj):
    ...
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.scatter(x, y, c=list(map(lambda x: x / max(door1_distances), door1_distances)), cmap=plt.cm.bone,
                norm=matplotlib.colors.LogNorm())
    plt.colorbar()
    plt.title("Distance to 1st Door in Current Room")

    plt.subplot(1, 2, 2)
    plt.scatter(x, y, c=list(map(lambda x: x / max(door2_distances), door2_distances)), cmap=plt.cm.bone,
                norm=matplotlib.colors.LogNorm())
    plt.colorbar()
...
def plot_door_angles(room_number):
    ...
    plt.suptitle("Agent Space: Angle to Doors")
    plt.show()
    plt.close()
