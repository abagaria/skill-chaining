...
"""
What's the plan?
The plan is, collect data the normal way. Collect it along with the position.
Find out exactly what is the center point. Then, we'll just train something where
you get a list of states/positions, calculate a label based on the states/positions,
and then train a classifier with the DQN architecture to do as well as it can on them.

Then, we see: how easy is the task? How many gradient steps does it take? Can it do it at all?

Actually, as an even more straightforward plan, I think I can just overwrite the reward function,
...
class Experiment15:
    """
    GREEDY RL where the reward is to push in the direction of the velocity.
    Made to determine how long we would expect training to take if it were 
    the PERFECT supervisory signal.
    """
    ...
    def train_dqn_agent(self, agent, mdp, episodes, steps):
        ...
        for episode in range(episodes):
            ...
            for step in range(steps):
                ...
                true_reward = 20. if next_state.is_terminal() else 0.
                # reward_signal = position
                vel_left = position[1] < 0
                if action == 1:
                    reward_signal = 0.
                elif action == 0:
                    reward_signal = 1. if vel_left else -1.
                elif action == 2:
                    reward_signal = -1. if vel_left else 1.
                else:
                ...
                agent.step(state.features(), position, action, reward_signal, next_state.features(), next_position,
            ...
            print(f"Reward Signal: {correct_guess}")
...
if __name__ == "__main__":
    ...
    """
    python experiment15.py --exploration_method eps-const --pixel_observation --experiment_name testing --run_title testing
    """
    ...
    exp = Experiment15(args.seed, pixel_observation=args.pixel_observation,
                      eval_eps=args.eval_eps,
                    #   exploration_method=args.exploration_method,
                      exploration_method="eps-const",
                      num_episodes=args.episodes, tensor_log=args.tensor_log,
