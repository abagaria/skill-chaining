...
def plot_two_class_classifier_for_block_pos(option, block_position, plot_examples=True, seed=0):
    states = get_grid_states(option.overall_mdp, block_position)
    values = get_initiation_set_values(option, block_position)
    ...
    plt.colorbar()
    plt.title(f"Block at {block_position}")
...
def plot_two_class_classifier(option, episode, experiment_name, plot_examples=True, seed=0):
    block_positions = [(0, 6), (1, 6), (2, 6), (3, 6), (4, 6), (5, 6)]
    
    plt.figure(figsize=(16, 12))

    for i, block_pos in enumerate(block_positions):
        plt.subplot(2, 3, i+1)
        plot_two_class_classifier_for_block_pos(option, block_pos, plot_examples, seed)
    
    plt.savefig(f"initiation_set_plots/{experiment_name}/{option.name}-pessimistic-init-classifier-{episode}.png")
    plt.close()
...
def plot_optimistic_classifier(option, experiment_name, chunk_size=1000):
    states = option.solver.replay_buffer.obs2_buf
    values = np.array((states.shape[0],))
    ...
    current_idx = 0
    for chunk_number, state_chunk in tqdm(enumerate(state_chunks)):
        init_values = option.optimistic_classifier.predict(state_chunk) == 1
        current_chunk_size = len(state_chunk)
        values[current_idx:current_idx + current_chunk_size] = init_values
        current_idx += current_chunk_size
    plt.scatter(states[:, 0], states[:, 1], c=values)
    print(values.sum())
    plt.colorbar()
    plt.savefig(f"initiation_set_plots/{experiment_name}/{option.name}-initiation-classifier.png")
    plt.close()

def plot_pessimistic_classifier(option, experiment_name, chunk_size=1000):
    states = option.solver.replay_buffer.obs2_buf
    values = np.array((states.shape[0],))
    num_chunks = int(np.ceil(states.shape[0] / chunk_size))
    if num_chunks == 0: return 
    ...
    current_idx = 0
    for chunk_number, state_chunk in tqdm(enumerate(state_chunks)):
        state_chunk = state_chunk[:, :4]
        init_values = option.pessimistic_classifier.predict(state_chunk) == 1
        current_chunk_size = len(state_chunk)
        values[current_idx:current_idx + current_chunk_size] = init_values
        current_idx += current_chunk_size
    plt.scatter(states[:, 0], states[:, 1], c=values)
    print(f"Number of states inside pessimistic classifier: {values.sum()}/{len(values)}")
    plt.colorbar()
    plt.savefig(f"initiation_set_plots/{experiment_name}/{option.name}-pessimistic-initiation-classifier.png")
    plt.close()
