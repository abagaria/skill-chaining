...
class Option(object):
        ...
        def __init__(self, overall_mdp, name, global_solver, lr_actor, lr_critic, ddpg_batch_size, classifier_type="ocsvm",
                                 ...
                                 dense_reward=False, enable_timeout=True, timeout=100, initiation_period=2, pixel_observations=False, random_agent=False,
                ...
                if overall_mdp.is_action_space_discrete():
                        solver_name = "{}_dqn_agent".format(self.name)

                        if self.random_agent:
                                self.global_solver = RandomAgent(action_size, name=solver_name) if name == "global_option" else global_solver
                        else:
                                self.global_solver = DQNAgent(state_size, action_size, [], seed, device, name=solver_name, pixel_observation=pixel_observations) if name == "global_option" else global_solver
        ...
        def train_one_class_svm(self):
                ...
                # TODO: SVM require 2-dimensional array.
                positive_examples = np.array(self.positive_examples)
                positive_examples = positive_examples.reshape((len(self.positive_examples), -1))
                # print('positive_examples.shape=', positive_examples.shape)
                # print('One class SVM: flattened input')
...
#         def __init__(self, overall_mdp, name, global_solver, lr_actor, lr_critic, ddpg_batch_size, classifier_type="ocsvm",
#                                  subgoal_reward=0., max_steps=20000, seed=0, parent=None, num_subgoal_hits_required=3, buffer_length=20,
#                                  dense_reward=False, enable_timeout=True, timeout=100, initiation_period=2, random_agent=False,
#                                  generate_plots=False, device=torch.device("cpu"), writer=None):
#                 '''
#                 Args:
#                         overall_mdp (MDP)
...
#                 solver_name = "{}_ddpg_agent".format(self.name)
# 
#                 if not overall_mdp.is_action_space_discrete():
#                         # Continuous                 
#                         if self.random_agent:
...
#                 else:
#                         # Discrete
#                         # TODO: set pixel_observation
#                         if self.random_agent:
#                                 self.global_solver = RandomAgent(action_size, name=solver_name) if name == "global_option" else global_solver
#                         else:
#                                 self.global_solver = DQNAgent(state_size, action_size, [], seed, device, name=solver_name, pixel_observation=True) if name == "global_option" else global_solver
# 
#                         self.solver = DQNAgent(state_size, action_size, [], seed, device, tensor_log=(writer is not None), writer=writer, name=solver_name, pixel_observation=True)
#                 # Attributes related to initiation set classifiers
#                 self.num_goal_hits = 0
#                 self.positive_examples = []
#                 self.negative_examples = []
