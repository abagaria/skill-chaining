...
def render_states_with_max_val(agent, states):
    states = np.array([np.array(state.features()) for state in states])
...
def make_chunked_value_function_plot(solver, episode, seed, experiment_name, state_buffer, chunk_size=1000):
    obs = np.array([np.array(state.features()) for state in state_buffer])
...
def make_chunked_goal_conditioned_value_function_plot(solver, state_buffer, goal, episode, seed, experiment_name, chunk_size=1000):
    truncated_states = [np.array(state.features())[:4, ...] for state in state_buffer]

    obs = np.array([np.concatenate((state, goal.get_frame(-1)), axis=0) for state in truncated_states])
    pos = np.array([state.position for state in state_buffer])
    ...
    for state_chunk in tqdm(state_chunks, desc="Making VF plot"):  # type: (int, np.ndarray)
        state_chunk = torch.from_numpy(state_chunk).float().to(solver.device)
        chunk_qvalues = solver.get_batched_qvalues(state_chunk).cpu().numpy()
        chunk_qvalues = np.max(chunk_qvalues, axis=1)
        current_chunk_size = len(state_chunk)
...
def get_goal(agent):
    ex = agent.agent.replay_buffer.sample(1000)
    ex = list(itertools.chain.from_iterable(ex))
    states = [exp['next_state'] for exp in ex if exp['is_state_terminal']]
