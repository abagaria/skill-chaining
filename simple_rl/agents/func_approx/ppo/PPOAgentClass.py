...
class PPOAgent(object):
    def __init__(self,
        ...
        self.obs_normalizer = pfrl.nn.EmpiricalNormalization(
            obs_size  # Not using a clip threshold because the maze is larger than it
        )
...
def make_chunked_vf_plot(solver, experiment_name, episode):
    ...
    values = chunked_inference(solver.value_function, states, device=agent.device)
...
def train(agent, rnd, mdp, episodes, steps, experiment_name, starting_episode=0):

    def _get_intrinsic_reward(s):
        return rnd.get_reward(s.features()[None, ...]).detach().cpu().numpy()[0]
    ...
    for episode in range(starting_episode, starting_episode + episodes):
        ...
        for step in range(steps):
            ...
            rnd.update(_features(next_state))
        ...
        if rnd.use_reward_norm:
            rnd.update_reward_rms(episodic_rewards)
        ...
        print(f"Episode: {episode} | Intrinsic Reward: {intrinsic_score} | Var: {rnd.reward_rms.var} | Final State: {final_state}")
...
if __name__ == "__main__":
    ...
    reward_learner = RND(mdp.state_space_size(),
                         lr=1e-4,
                         n_epochs=1,
                         batch_size=64,
                         device=args.device,
    ...
    pes, ped, pei = train(agent, reward_learner, mdp, args.episodes, args.steps, args.experiment_name)
